            ...
            <Card key={questionData.id} className={cn("overflow-hidden", isCorrect ? 'border-l-4 border-green-500' : 'border-l-4 border-red-500')}>
                <CardHeader className="flex flex-row justify-between items-start">
                    ...
                    <div className="flex items-center gap-2">
                        <Button variant="neumorphic" size="sm" onClick={() => toggleTag(questionData, 'bookmarked')} isSelected={(questionData.tags || []).includes('bookmarked')}>
                            Bookmark
                        </Button>
                        <Button variant="neumorphic" size="sm" onClick={() => toggleTag(questionData, 'hard')} isSelected={(questionData.tags || []).includes('hard')} isDestructive={(questionData.tags || []).includes('hard')}>
                            Mark as Hard
                        </Button>
                        <Button variant="neumorphic" size="sm" onClick={() => toggleTag(questionData, 'revise')} isSelected={(questionData.tags || []).includes('revise')}>
                            Revise Later
                        </Button>
                    </div>
                </CardHeader>
