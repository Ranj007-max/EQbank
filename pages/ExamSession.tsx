...
import { ChevronLeft, ChevronRight, Flag, Power, LoaderCircle, MoreVertical } from 'lucide-react';
      ...
      <header className="flex items-center justify-between p-2 border-b border-gray-200 dark:border-gray-800 flex-shrink-0 bg-white dark:bg-gray-950 h-20">
        ...
        <div className="relative">
          <Button variant="ghost" size="icon" onClick={() => setIsMenuOpen(prev => !prev)}>
            <MoreVertical size={20} className="text-accent-violet" />
          </Button>
          {isMenuOpen && (
      ...
      <div className="flex flex-grow overflow-hidden">
        ...
        <main className="flex-grow p-6 overflow-y-auto">
          <div key={currentIndex} className="animate-question-change">
            <Card className="flex-grow flex flex-col bg-white dark:bg-gray-950 rounded-xl shadow-lg">
              <CardHeader>
            ...
            <CardContent>
                ...
                {currentQuestion.options.map((option, i) => (
                    <Button
                      ...
                      className={cn(
                        "w-full h-auto justify-start p-4 text-base whitespace-normal rounded-lg transition-all",
                        "bg-option-unselected-bg text-option-unselected-text hover:bg-option-hover-bg",
                        sessionState.answers[currentQuestion.id] === option && "bg-blue-500 text-white"
                      )}
                    >
        ...
        <aside className="w-full lg:w-80 flex-shrink-0 border-l border-gray-200 dark:border-gray-800 p-4 flex-col bg-white dark:bg-gray-950 hidden lg:flex">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-foreground text-lg">Navigator</h3>
            <Button onClick={toggleMarkForReview} variant={sessionState.markedForReview.includes(currentQuestion.id) ? 'warning' : 'outline'} size="sm" className="gap-2">
              <Flag size={16}/>
              {sessionState.markedForReview.includes(currentQuestion.id) ? 'Marked' : 'Mark'}
            </Button>
          </div>
          <div className="grid grid-cols-6 gap-2 flex-grow overflow-y-auto content-start p-2 bg-gray-100 dark:bg-gray-900 rounded-lg">
            {sessionState.questions.map((q, index) => {
              ...
              const statusClasses = {
                current: 'ring-2 ring-primary ring-offset-2 ring-offset-background dark:ring-offset-gray-950',
                answered: 'bg-blue-500 hover:bg-blue-600 text-white',
                marked: 'bg-red-500 hover:bg-red-600 text-white',
                skipped: 'bg-yellow-400 hover:bg-yellow-500 text-black',
                default: 'bg-gray-300 dark:bg-gray-700 hover:bg-gray-400 dark:hover:bg-gray-600'
              };

              let statusClass = statusClasses.default;
              if (sessionState.visited.includes(q.id) && !isAnswered) statusClass = statusClasses.skipped;
              if (isAnswered) statusClass = statusClasses.answered;
              if (isMarked) statusClass = statusClasses.marked;

              return (
                <button key={q.id} onClick={() => changeQuestion(index)} className={cn("h-10 w-10 rounded-full flex items-center justify-center font-bold transition-all duration-200 text-sm", statusClass, isCurrent && statusClasses.current)}>
                  {index + 1}
                </button>
              );
            })}
