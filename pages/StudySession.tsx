      ...
      <Card className="overflow-hidden">
        ...
        <CardContent className="p-6">
            ...
            {currentQuestion.options.map((option, i) => {
                ...
                <Button
                    ...
                    variant="neumorphic"
                    selected={!!(isSelected || (selectedOption && isCorrect))}
        ...
        <CardFooter className="bg-muted/30 border-t px-6 py-3 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <Button id={`ss-bookmark-${currentQuestion.id}`} variant="neumorphic" size="sm" onClick={() => toggleTag('bookmarked')} selected={(currentQuestion.tags || []).includes('bookmarked')}>Bookmark</Button>
            <Button id={`ss-hard-${currentQuestion.id}`} variant="neumorphic" size="sm" onClick={() => toggleTag('hard')} selected={(currentQuestion.tags || []).includes('hard')} isDestructive={(currentQuestion.tags || []).includes('hard')}>Mark as Hard</Button>
            <Button id={`ss-revise-${currentQuestion.id}`} variant="neumorphic" size="sm" onClick={() => toggleTag('revise')} selected={(currentQuestion.tags || []).includes('revise')}>Revise</Button>
          </div>
          {selectedOption && (
